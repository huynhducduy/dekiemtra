...
echo "
                ...
                <h2><a href='#'>Bảng điều khiển chính</a> &raquo; <a href='#' class='active'>Đăng đề trắc nghiệm - Bước 1</a>
				<input type='text' placeholder='Tìm kiếm' name='key' class='search' value='".$_GET['key']."'/>
				</h2></form>
                <div id='main'>
				<br/>
				Chọn một đề kiểm tra chưa có đề trắc nghiệm<br/><br/>
...
if (isset($_GET['key']))
{
$sql="SELECT * FROM `tests` where `rt`=0 and `keyword` LIKE '%".mysql_escape_string($_GET['key'])."%'";
}
else
{
$sql="SELECT * FROM `tests` where `rt`=0";
}
...
if (isset($_GET['key']))
{
$sql2="SELECT * FROM `tests` where `rt`=0 and `keyword` LIKE '%".mysql_escape_string($_GET['key'])."%' order by `id` DESC LIMIT ".$start.",".$display; // Lấy CSDL
}
else
{
$sql2="SELECT * FROM `tests` where `rt`=0 order by `id` DESC LIMIT ".$start.",".$display; // Lấy CSDL
}
...
$next=$p+1;
	...
	if ($p < $page)
	{
		echo "<li><b><a href='./tposttest.php?p=".$next."&key=".$_GET[key]."' title='Trang ".$next."' alt='Trang ".$next."'>".$next."</a></b></li>&nbsp;";
		echo "<li><b><a href='./tposttest.php?p=".$next."&key=".$_GET[key]."' title='Trang sau - ".$next."' alt='Trang sau - ".$next."'><font face='arial'>►</font></a></b></li>&nbsp;";
	}
	if ($p+1 < $page)
	{
		echo "<li><b><a href='./tposttest.php?p=".$page."&key=".$_GET[key]."' title='Trang cuối cùng - ".$page."' alt='Trang cuối cùng - ".$page."'>&raquo;</a></b></li>";
	}
echo "</ul>";
}
else
...
$next=$p+1;
	if ($p-1 > 1)
	{
		echo "<li><b><a href='./tposttest.php?p=' title='Trang đầu tiên - 1'>&laquo;</a></b></li>&nbsp;";
	} 
	if ($p > 1)
	{ 
		echo "<li><b><a href='./tposttest.php?p=".$prev."' title='Trang trước - ".$prev."' alt='Trang trước - ".$prev."'><font face='arial'>◄</font></a></b></li>&nbsp;";
		echo "<li><b><a href='./tposttest.php?p=".$prev."' title='Trang ".$prev."' alt='Trang ".$prev."'>".$prev."</a></b></li>&nbsp;";
	}
	echo "<li><b><a title='Trang hiện tại' alt='Trang hiện tại' class='current'>".$p."</a></b></li>&nbsp;";
	if ($p < $page)
	{
		echo "<li><b><a href='./tposttest.php?p=".$next."' title='Trang ".$next."' alt='Trang ".$next."'>".$next."</a></b></li>&nbsp;";
		echo "<li><b><a href='./tposttest.php?p=".$next."' title='Trang sau - ".$next."' alt='Trang sau - ".$next."'><font face='arial'>►</font></a></b></li>&nbsp;";
	}
	if ($p+1 < $page)
	{
		echo "<li><b><a href='./tposttest.php?p=".$page."' title='Trang cuối cùng - ".$page."' alt='Trang cuối cùng - ".$page."'>&raquo;</a></b></li>";
	}
echo "</ul>";
}
}
